...
public class ResultComputing {
    ...
    public static List<Double> compute(int runs, Individual bestIndividual, int dimension, int individualCount, double crossoverProbability, double F, int maxFFE, FitnessFunction ffunction, Boundary boundaryRange, double ffOptimum, double fixedAccuracyLevel){
        ...
        DErand1bin de;
        Sink sink;
        Sink[] sinks = new Sink[runs];
        
        
        TreeMap<SinkKeysEnum, Object> desc = new TreeMap<>();
        
        desc.put(SinkKeysEnum.ALGORITHM,"DErand1bin");
        desc.put(SinkKeysEnum.DIMENSION,dimension);
        desc.put(SinkKeysEnum.POPSIZE,individualCount);
        desc.put(SinkKeysEnum.CR,crossoverProbability);
        desc.put(SinkKeysEnum.F,F);
        desc.put(SinkKeysEnum.CFE_MAX,maxFFE);
        desc.put(SinkKeysEnum.FAL,fixedAccuracyLevel);
        desc.put(SinkKeysEnum.FITNESS_FUNCTION, ffunction.getName());
        
        for(int i=0; i<runs; i++){
            
            sink = new WolframSink(desc);
            de = new DErand1bin(bestIndividual, dimension, individualCount, crossoverProbability, F, maxFFE, ffunction, boundaryRange, ffOptimum, fixedAccuracyLevel,sink);
            de.run();
            sinks[i] = de.getSink();
...
//        srTimesSs = srate * sspeed;
        ...
        DescriptiveStatistics stat = new DescriptiveStatistics(Worker.getBestValues(sinks));
        resultList.add(stat.getMean());
    ...
    public static void computeAndPrintOutCRFs(String fileName, double Fmin, double Fmax, double Fstep, double CRmin, double CRmax, double CRstep, int runs, Individual bestIndividual, int dimension, int individualCount, int maxFFE, FitnessFunction ffunction, Boundary boundaryRange, double ffOptimum, double fixedAccuracyLevel){
        ...
        for(List<Double> list: resList){
            ...
            normalized.add((list.get(5)-min)/(max-min));
        }
...
//        PrintWriter writer3;
        try {
            writer = new PrintWriter("HM-"+fileName, "UTF-8");
...
//                System.out.println("SRate x SSpeed: " + list.get(2));
                ...
                writer.print("{" + list.get(3) + "," + list.get(4) + "," + String.format(Locale.ENGLISH,"%.10f", normalized.get(i)) + "}");
...
//            writer3.close();
    ...
    public static void main(String[] args) {
      ...
      /**
       ...
       features = new double[]{-61.67713722207517, -7.684525402507525, 87.94238891851492, -43.54210399352583, -62.59488750718182, 97.35187950444471, 68.33052512544774, 10.072597485805419, 56.10503687634504, -69.18168336936716};
       ff = new f1();
       ff.init(10);
       ind = new Individual(features, ff.getValue(features));
       ffo = ff.getOptimum(D);
